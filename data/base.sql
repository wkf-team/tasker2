...
CREATE OR REPLACE VIEW `v_goals_complete` AS select `goals`.`id` AS `id`,`goals`.`subject` AS `subject`,`goals`.`project_id` AS `project_id`,(((((((count(distinct `tasks`.`id`) + count(distinct `tasks1`.`id`)) + count(distinct `tasks2`.`id`)) + count(distinct `tasks3`.`id`)) + count(distinct `tasks4`.`id`)) + count(distinct `tasks5`.`id`)) + count(distinct `tasks6`.`id`)) + count(distinct `tasks7`.`id`)) AS `total`,(((((((sum(if((`tasks`.`status_id` >= 6),1,0)) + sum(if((`tasks1`.`status_id` >= 6),1,0))) + sum(if((`tasks2`.`status_id` >= 6),1,0))) + sum(if((`tasks3`.`status_id` >= 6),1,0))) + sum(if((`tasks4`.`status_id` >= 6),1,0))) + sum(if((`tasks5`.`status_id` >= 6),1,0))) + sum(if((`tasks6`.`status_id` >= 6),1,0))) + sum(if((`tasks7`.`status_id` >= 6),1,0))) AS `closed`,`goals`.`due_date` AS `due_date` from ((((((((`ticket` `goals` left join `ticket` `tasks` on((`tasks`.`parent_ticket_id` = `goals`.`id`))) left join `ticket` `tasks1` on((`tasks1`.`parent_ticket_id` = `tasks`.`id`))) left join `ticket` `tasks2` on((`tasks2`.`parent_ticket_id` = `tasks1`.`id`))) left join `ticket` `tasks3` on((`tasks3`.`parent_ticket_id` = `tasks2`.`id`))) left join `ticket` `tasks4` on((`tasks4`.`parent_ticket_id` = `tasks3`.`id`))) left join `ticket` `tasks5` on((`tasks5`.`parent_ticket_id` = `tasks4`.`id`))) left join `ticket` `tasks6` on((`tasks6`.`parent_ticket_id` = `tasks5`.`id`))) left join `ticket` `tasks7` on((`tasks7`.`parent_ticket_id` = `tasks6`.`id`))) where (((`goals`.`status_id` < 6) or ((`goals`.`status_id` = 6) and (`goals`.`end_date` >= (now() - interval 14 day)))) and (`goals`.`ticket_type_id` = 1)) group by `goals`.`id` order by ifnull(`goals`.`due_date`,'3000-01-01');
...
CREATE  OR REPLACE VIEW `v_users_balance` AS select `t`.`owner_user_id` AS `owner_user_id`,`user`.`name` AS `user_name`,count(`t`.`id`) AS `total`,sum(`t`.`estimate_time`) AS `sum_time` from (`ticket` `t` left join `user` on((`user`.`id` = `t`.`owner_user_id`))) where ((`t`.`status_id` < 6) and (`t`.`ticket_type_id` > 1)) group by `t`.`owner_user_id`;
